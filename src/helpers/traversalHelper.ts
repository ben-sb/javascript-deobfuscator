...
export default class TraversalHelper {
     ...
     */
    static replaceNode(root: Shift.Node, node: Shift.Node, replacement: Shift.Node | Shift.Node[] | null): void {
        traverse(root, {
                    ...
                    for (const prop of Object.getOwnPropertyNames(parent)) {
                        ...
                        if (value == n) {
                            if (Array.isArray(replacement)) {
                                throw new Error(`Cannot insert array of nodes into property ${prop} of ${parent.type}`);
                            }
